...
When we use N = 5,000,000 and searches for the last element instead of a random target. This is a worst-case target for linear search because it must check every element before finding the match, while binary search still only needs about `log2(N)` comparisons.
